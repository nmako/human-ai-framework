...
A note on tool building and commercial use: building a tool from this specification for your own organization's internal AI governance work is permitted. Building a tool that you offer commercially to third parties (productized SaaS, paid consulting offering, commercial training program) requires a separate commercial license. See the License section below.
...
The repository is open for contribution under the licensing terms specified in `LICENSE`. Contributions are welcome in the following areas:
...
- **Tool building feedback.** Sharing observations from implementing the framework as a working tool. (Note: building commercial tools from the specification requires a separate license — see License section.)
- **Documentation and accessibility.** Translations, plain-language explainers, visual diagrams.

Contribution guidelines are documented in `CONTRIBUTING.md`. By contributing, you agree to license your contributions under the same CC BY-NC-SA 4.0 license as the framework.
...
The framework is released under **Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International (CC BY-NC-SA 4.0)** for non-commercial use, with separate commercial licenses available on request.

**Non-commercial use** includes: reading and sharing the framework; using it for your own organization's internal AI governance work; academic citation and research; NGO advocacy and policy analysis; regulatory consultation; teaching the framework in university courses or internal staff training. No separate authorization is required for these uses.

**Commercial use** requires a separate license. This includes: consulting practices using the framework as a substantive basis for client engagements; training programs or certifications derived from the framework; productized tools or platforms built from the specification and offered commercially to third parties; commercial publication of framework content beyond fair-use academic citation.

To inquire about a commercial license, contact **natalia@humanaipolicy.org**. See `LICENSE` for the full terms, edge case clarifications, and commercial use guidance.

---

## How to engage

- **For tool building:** Read the specification, build the tool, and share back what you learn. The framework's design assumes its operational behavior will be tested through actual tool implementation. Tools built for your organization's internal use are permitted under the non-commercial license; commercial tool offerings require a separate license (see License section).
...
- **For pilot testing:** Contact natalia@humanaipolicy.org.
...
*Framework specification version 1.0. Last updated: May 2026. For questions, see GitHub issues or contact natalia@humanaipolicy.org.*
